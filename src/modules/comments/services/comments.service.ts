...
export class CommentsService {
	...
	async findById(id:string): Promise<Comment> {
		const comment = await this.commentModel.findById(id).populate("user", "", this.userModel);
		if(!comment) {
  ...
  async create(data: CommentDto): Promise<Comment> {
	...
	async update(id: string, comment: EditCommentDto): Promise<Comment> {
		const commentUpdate = await this.commentModel.findByIdAndUpdate(id, comment, { new: true });
